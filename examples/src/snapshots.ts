import { Sandbox, type Snapshot } from "@buddy-works/sandbox-sdk";
...
try {
	...
	log(
		`Created base sandbox: ${base.data.identifier} (${base.data.html_url})\n`,
	);
	...
	snapshot = await base.createSnapshot({ name: "demo-snapshot" });
	...
	restored = await Sandbox.createFromSnapshot(snapshot.id, {
	...
	await base?.destroy().catch(() => undefined);
